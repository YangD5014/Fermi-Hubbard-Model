...
from HW_FermiHubbard import FermiHubbard,convert_hamiltonian_PauliOP,depth_cir
...
class FH_AVQDS():
    '''
    8-sites Fermi-Hubbard model
    由于 16 比特 已经无法得知真实的 exact value anymore
    
    '''
    def __init__(self,EndTime:float,N_site:int,U:float,J:float,Initial_circuit:Circuit,TimeStep:float=0.005,Boundary:str="Hardwall",Threshold: float=1e-3,max_add:int=2,max_theta:float=0.005,dt_max:float=0.02) -> None:
        self.N_site = N_site
        self.U = U
        self.J = J
        self.initial_state_circuit = Initial_circuit
        self.Boundary = Boundary
        self.model = FermiHubbard(N_site=self.N_site,U=self.U,J=self.J)
        self.Hamiltonian_initial = self.model.HW_hamiltonian
        self.H = self.Hamiltonian_initial
        self.n_qubit = self.Hamiltonian_initial.count_qubits()
        self.initial_state = self.initial_state_circuit
    ...
    def HamiltonianPool_Init(self):
        self.model.Hamiltonian_pool_init()
        self.HamiltonianPoolOp=[]
        for i in self.model.HamiltonianPoolOp:
            each_op = TimeEvolution(ops=convert_hamiltonian_PauliOP(i)).circuit
            self.HamiltonianPoolOp.append(each_op)
            
        
    def FirstStep(self):
        self.logger.info(f'-----------------第{self.general_step}/个时间步长,当前时间是{self.currentTime}-------------------')
        self.currentTime = 0.0
        self.currentAnsatz = Circuit()
        self.currentAnsatz.extend(self.initial_state)
        self.currentIndex = 0
