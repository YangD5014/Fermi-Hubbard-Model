...
class LMS_AVQDS_Energy():
    def __init__(self,EndTime:float,N_site:int,T:int,hz:float,TimeStep: float=0.01,Threshold: float=1e-3,max_add:int=2,max_theta: float=0.005,growing_mode:int=1,dt_max:float=0.1) -> None:
        self.N_site = N_site
        self.T = T
        self.hz=hz
        self.Hamiltonian_initial = LMSHamiltonian(N_site=N_site,gamma=gamma(t=0.0,T=T),h_z=hz)
